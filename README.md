# 🌌 Naman Sharma

### SYSTEM ONLINE

> Exploring the space between software, security, and low-level systems.

I'm a Computer Science student focused on understanding how technology works beneath the surface — building tools, analyzing systems, and exploring security.

## 🛰️ Current Mission
[+] Decode unknown systems
[+] Analyze software behaviour
[+] Discover vulnerabilities
[+] Build useful tools
[+] Improve every day

## ⚡ Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=cpp,c,python,bash,linux,git" />

</div>

## 🔬 Research Log

| Area | Exploring |
|---|---|
| 🛡 Security | Bug bounty, vulnerabilities, attack surfaces |
| 🧩 Reverse Engineering | ELF binaries, debugging, analysis |
| 🚘 Automotive | CAN Bus, UDS, ECU security |
| 🛠 Automation | Python tools and scripting |

## 🚀 Projects

### 🔐 CipherIt
File encryption/decryption utility built with Python.

### 🚗 Automotive Security Research
Exploring CAN communication, UDS services, and ECU behaviour.

### 🧩 Reverse Engineering Lab
Learning binary analysis and system internals.

## 📡 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/Naman_Sharma-Yup)

[![Email](https://img.shields.io/badge/Email-red?style=for-the-badge&logo=gmail)](mailto:ns580654@gmail.com)

## 📊 GitHub Stats

![Stats](https://github-readme-stats.shion.dev/api?username=learningcoder05&theme=tokyonight&hide_border=true)

![Streak](https://streak-stats.demolab.com/?user=learningcoder05&theme=tokyonight&hide_border=true)

---

<div align="center">

"Curiosity is the first exploit."

![Profile Views](https://komarev.com/ghpvc/?username=learningcoder05&style=flat-square)

</div>
